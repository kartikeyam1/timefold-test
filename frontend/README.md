...
- **👁️ Rider Management Panel**: 
  - Individual rider visibility toggles
...
5. **In Output view**: Use the Rider Management Panel to:
   - Toggle individual riders on/off with the 👁️ button
   - Use "Show All" / "Hide All" for bulk operations
   - Click "High Only" to show only highly utilized riders (70%+)
   - View utilization bars and efficiency metrics
...
- **Rider Management**: Advanced controls for selective visualization

## 🚚 Rider Management Features

### **Individual Control**
- **👁️ Toggle Button**: Click the eye icon to show/hide each rider
- **Utilization Bar**: Visual representation of capacity usage
- **Efficiency Metrics**: Orders assigned vs. capacity
- **Smart Sorting**: Riders sorted by utilization (highest first)

### **Bulk Operations**
- **Show All**: Make all riders visible on the map
- **Hide All**: Hide all riders for clean map view
- **High Only**: Show only riders with 70%+ utilization
- **Collapsible Panel**: Minimize/expand the rider list

### **Real-time Updates**
- **Live Counter**: Shows visible/total riders in header
- **Instant Feedback**: Immediate map updates when toggling
- **Persistent State**: Visibility preferences maintained during filtering
